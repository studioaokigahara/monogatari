...
def check_checkpoint_args(checkpoint_args):
    ...
    def _compare(arg_name, old_arg_name=None):
        ...
        assert (
            checkpoint_value == args_value
        ), f"{arg_name} value from checkpoint ({checkpoint_value}) \
                is not equal to the input argument value ({args_value})."
...
def read_metadata(tracker_filename):
    ...
    if torch.distributed.is_initialized():
        ...
        if iteration != max_iter:
            print(
                f"WARNING: on rank {rank} found iteration {iteration} in the \
                metadata while max iteration across the ranks \
                is {max_iter}, replacing it with max iteration."
            )
    else:
...
def save_checkpoint(iteration, model, optimizer, opt_param_scheduler):
    ...
    if args.deepspeed:
        save_deepspeed_checkpoint(iteration, model, args)
    ...
    if (
        ...
        model_state_dict["tokens"] = args.consumed_train_tokens
        if not args.deepseed:
            if len(model) == 1:
    ...
    if not args.deepspeed:
        ...
        model[0].save_checkpoint(args.save, iteration, state_dict)
    ...
    print_rank_0(
        f"Successfully saved checkpoint at iteration {iteration:7d} to {args.save}"
    )
...
def save_deepspeed_checkpoint(iteration, model, args):
    """Save a model checkpoint using the DeepSpeed checkpointing API."""
    ...
    model.save_checkpoint(args.save, iteration, client_state=ds)
...
def _load_base_checkpoint(load_dir, use_distributed_optimizer, rank0=False):
    ...
    if not os.path.isfile(tracker_filename):
        if not rank0:
            print_rank_0(
                f"Could not find the metadata file {load_dir}, \
                will start from random instead of loading a checkpoint"
            )
...
def load_checkpoint(
    ...
    if args.deepspeed:
        if args.finetune:
            loaded_dir, state_dict = model[0].load_checkpoint(
                load_dir, strict, False, False, True
            )
        else:
            loaded_dir, state_dict = model[0].load_checkpoint(load_dir, strict)
    ...
    # Set iteration.
    if args.finetune or release or args.reset_iteration or load_weights_only:
        iteration = 0
        ...
        try:
            ...
            try:  # Backward compatible with older checkpoints
                ...
                print_rank_0(
                    f"A metadata file exists but unable to load \
                    iteration from checkpoint {checkpoint_name}, exiting..."
                )
                sys.exit()

    if args.deepspeed:
        checkpoint_name, ds = model.load_checkpoint(args.load, iteration)
    ...
    # Check arguments.
    if not load_weights_only and not args.reset_iteration:
    ...
    if not release and not args.finetune and not args.no_load_rng:
        try:
            if "rng_state" in model_state_dict:
                ...
                get_accelerator().set_rng_state(model_state_dict["cuda_rng_state"])
                # Check for empty states array
