...
def _vocab_size_with_padding(orig_vocab_size, args):
    ...
    print_rank_0(
        ...
        {after - orig_vocab_size} dummy tokens. (new size: {after})"
    )
    return after
...
class AbstractTokenizer(ABC):
    ...
    def detokenize(self, token_ids):
        raise NotImplementedError(
            f"Detokenization is not implemented for {self.name} tokenizer"
        )

    @property
    def cls(self):
        raise NotImplementedError(f"CLS is not implemented for {self.name} tokenizer")

    @property
    def sep(self):
        raise NotImplementedError(f"SEP is not implemented for {self.name} tokenizer")

    @property
    def pad(self):
        raise NotImplementedError(f"PAD is not implemented for {self.name} tokenizer")

    @property
    def eod(self):
        raise NotImplementedError(f"EOD is not implemented for {self.name} tokenizer")

    @property
    def mask(self):
        raise NotImplementedError(f"MASK is not implemented for {self.name} tokenizer")
...
class _YTTMTokenizer(AbstractTokenizer):
    ...
    def _initalize(self, vocab_extra_ids):
        ...
        self.add_token("<CLS>")
        self.cls_id = self.vocab_["<CLS>"]
        self.add_token("<SEP>")
        self.sep_id = self.vocab_["<SEP>"]
        self.add_token("<PAD>")
        self.pad_id = self.vocab_["<PAD>"]
        self.add_token("<BOS>")
        self.bos_id = self.vocab_["<BOS>"]
        self.add_token("<EOS>")
        self.eos_id = self.vocab_["<EOS>"]
        self.add_token("<EOD>")
        self.eod_id = self.vocab_["<EOD>"]
        self.add_token("<MASK>")
        self.mask_id = self.vocab_["<MASK>"]
