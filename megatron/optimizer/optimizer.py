    ...
    def allreduce_layernorm_grads(self, args):
        ...
        if mpu.get_tensor_model_parallel_world_size() > 1 and args.sequence_parallel:
            ...
            for module in self.models:
                unwrapped_model = unwrap_model(
                    module, (torchDDP, LocalDDP, Float16Module)
                )
        ...
        for param_group in self.optimizer.param_groups:
            ...
            for i, param in enumerate(param_group["params"]):
                if param.requires_grad:
                    ...
                    # float16 params:
                    if param.type() in [
                        "get_accelerator().HalfTensor",
                        "get_accelerator().BFloat16Tensor",
                    ]:
                    ...
                    # fp32 params.
                    elif param.type() == "get_accelerator().FloatTensor":
                        fp32_params_this_group.append(param)
                        param_group["params"][i] = param

                    else:
                        raise TypeError(
                            f"Wrapped parameters must be one of \
                        get_accelerator().FloatTensor, \
                        get_accelerator().HalfTensor, or \
                        get_accelerator().BFloat16Tensor. Received {params.dtype()}"
                        )
