...
def load(args):
    ...
    # Fused softmax.
    extra_include_paths = (
        [os.path.abspath(srcpath)] if torch.version.hip is None else []
    )
    ...
    if args.masked_softmax_fusion:
        ...
        scaled_masked_softmax_cuda = _cpp_extention_load_helper(
            "scaled_masked_softmax_cuda", sources, extra_cuda_flag, extra_include_paths
        )
    ...
    extra_cuda_flags = ["-maxrregcount=50"] if torch.version.hip is None else []
