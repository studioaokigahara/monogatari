...
def build_num_microbatches_calculator(args):
    ...
    if args.rampup_batch_size is None:
        ...
        print_rank_0(
            f"Will use batch size rampup starting from \
            global batch size {start_batch_size} to global batch size \
            {args.global_batch_size} with batch size increments \
            {batch_size_increment} over {rampup_samples} samples."
        )
...
class ConstantNumMicroBatches(NumMicroBatchesCalculator):
    def __init__(self, global_batch_size, micro_batch_size, data_parallel_size):
        ...
        assert (
            ...
            micro batch size ({micro_batch_size}) times data parallel size ({data_parallel_size})"
...
class RampupBatchsizeNumMicroBatches(NumMicroBatchesCalculator):
    def __init__(
        ...
        assert (
            ...
            to be divisible by global batch size increment ({batch_size_increment})"
        ...
        if consistency_check:
            assert (
                ...
                == 0
                    ...
                    is not divisible by micro-batch-size ({self.micro_batch_size}) \
                    times data parallel size ({self.data_parallel_size})"
