...
def generate_tokens_probs_and_return_on_first_stage(
    ...
    if mpu.is_pipeline_last_stage():
        ...
        generated_sequence_lengths = (
            torch.ones(
                batch_size, dtype=torch.int64, device=get_accelerator().current_device()
            )
            * max_sequence_length
        )
    ...
    is_generation_done = torch.zeros(
        batch_size, dtype=torch.uint8, device=get_accelerator().current_device()
    )
...
def beam_search_and_return_on_first_stage(
    ...
    best_batches = None
    done = torch.zeros(1, dtype=torch.uint8, device=get_accelerator().current_device())
    scores = torch.zeros(
        beam_size, dtype=torch.float32, device=get_accelerator().current_device()
    ).unsqueeze(1)
            ...
            if mpu.is_pipeline_last_stage():
                ...
                if beam_hyp.is_done(
                    best_scores.max().item(), context_length + 1 - prompt_length
                        ...
                        1, dtype=torch.uint8, device=get_accelerator().current_device()
                    )
