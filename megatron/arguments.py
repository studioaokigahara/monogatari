...
def parse_args(extra_args_provider=None, ignore_unknown_args=False):
    ...
    parser = _add_monogatari_args(parser)
    ...
    args.ds_pipeline_enabled = args.pipeline_model_parallel_size > 1
...
def validate_args(args, defaults={}):
    ...
    ), f"world size ({args.world_size}) is not divisible by tensor model parallel size \
    ...
    ), f"world size ({args.world_size}) is not divisible by tensor parallel size \
            ({args.tensor_model_parallel_size}) times pipeline parallel size \
            ({args.pipeline_model_parallel_size})"
    args.data_parallel_size = args.world_size // model_parallel_size
    print_rank_0(
        f"Using world size: {args.world_size}, \
    ...
    if args.pipeline_model_parallel_size > 1:
        if args.pipeline_model_parallel_split_rank is not None:
            assert (
                ...
                < args.pipeline_model_parallel_size
            ), f"split rank needs to be less than pipeline model parallel size \
                            ({args.pipeline_model_parallel_size})"
    ...
    for key in defaults:
        ...
        if getattr(args, key) is not None:
            print_rank_0(
                ...
                {key}:{defaults[key]} with {key}:{getattr(args, key)}"
            )
        else:
    ...
    if args.global_batch_size is None:
        ...
        print_rank_0(f"setting global batch size to {args.global_batch_size}")
    ...
    if args.bf16:
        ...
        if not args.accumulate_allreduce_grads_in_fp32:
            ...
            print_rank_0("Accumulating and All-reducing gradients in fp32.")

    print_rank_0(f"Using dtype {args.params_dtype} for parameters...")
    ...
    if TORCH_MAJOR < 1 or (TORCH_MAJOR == 1 and TORCH_MINOR < 11):
        ...
        print_rank_0(
            "Persistent fused layer norm kernel is supported from pytorch v1.11 \
            (nvidia pytorch container paired with v1.11). \
    ...
    if args.distribute_saved_activations:
        ...
        ), f"Distributed recompute activations are supported for \
            PyTorch versions 1.10 and above (Nvidia Pytorch container >= 21.07). \
            Current PyTorch version is v{TORCH_MAJOR}.{TORCH_MINOR}."
    ...
    # args.sequence_parallel is enabled.
...
def _add_training_args(parser):
    ...
    group.add_argument(
        ...
        type=str,
        default="adam",
        choices=["adam", "sgd", "izanagi", "izanagizero"],
    ...
    group.add_argument(
        "--use-tutel", type=bool, default=False, help="Use Tutel optimization for MoE"
    )
    group.add_argument(
        "--inference",
        action="store_true",
...
def _add_distributed_args(parser):
    ...
    group.add_argument(
        ...
        type=bool,
        default=False,
        help="whether to use tensor parallelism for experts",
    )
    group.add_argument(
        "--distributed-backend",
        default="nccl",
    ...
    group.add_argument(
        ...
        type=int,
        choices=[0, 1, 2],
        help="Call empty_cache() each iteration "
...
def _add_data_args(parser):
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        help="SentencePiece/YTTM tokenizer model.",
    )
    group.add_argument(
        "--data-impl",
        type=str,
...
def _add_monogatari_args(parser):
    group = parser.add_argument_group(title="monogatari")
    ...
    group.add_argument(
        ...
        type=bool,
        default=True,
        help="Whether or not to use the Sub-LN architecture for the Feed-Forward and Attention blocks",
    )
    group.add_argument(
        "--xmoe",
        type=bool,
        default=True,
        help="Whether or not to use the routing improvements from X-MoE when using Mixture-of-Experts",
    )
    group.add_argument(
        "--expert-choice",
        type=bool,
        default=True,
        help="Whether or not to use the Expert Choice routing algorithm when using Mixture-of-Experts",
    )
    ...
    group.add_argument(
        ...
        type=int,
        default=32,
        help="Dimension to use for the EMA component when using Moving Average Equipped Gated Attention",
    )
    group.add_argument(
        "--mega-z-size",
        type=int,
        default=128,
        help="Dimension to use for the Z projection when using Moving Average Equipped Gated Attention. Equvialent to a shared QK projection in standard attention",
    )
    group.add_argument(
        "--mega-h-size",
        type=int,
        default=32,
        help="Dimension to use for the H projection when using Moving Average Equipped Gated Attention. Equvialent to the V projection in standard attention",
    )
    group.add_argument(
        "--mega-prenorm",
        type=bool,
    ...
    group.add_argument(
        ...
        type=int,
        default=32,
        help="Dimension to use for xPos position embeddings",
    )
    ...
    group.add_argument(
        ...
        type=bool,
        default=True,
        help="Use N-grammer layer if true",
    )
    group.add_argument(
        "--ngrammer-size",
        type=float,
        default=0.5,
        choices=[1.0, 0.5, 0.25, 0.125],
        help="Dimension to use for the N-Grammer layer as a percentage of the model hidden dimension",
    )
    group.add_argument(
        "--ngrammer-num-clusters",
        type=int,
        default=8192,
        help="Number of clusters to use, typically around the square root of the sequence length",
    )
    assert n and not (n & n - 1)
    group.add_argument(
        "--ngrammer-num-heads",
        type=int,
        default=8,
    ...
    assert n and not (n & n - 1)
    group.add_argument(
        "--ngrammer-dim-per-heads",
        type=int,
        default=8,
        help="Dimension to use per head",
    )
    group.add_argument(
        "--ngrammer-decay",
        type=float,
        default=0.999,
        help="The decay with which to update centroids",
    )
    group.add_argument(
        "--ngrammer-epsilon",
        type=float,
        default=1e-6,
        help="Tiny value to guard against division by zero",
    )
    group.add_argument(
        "--ngrammer-concat",
        type=bool,
    ...
    group.add_argument(
        ...
        type=float,
        default=0.999,
        help="Coefficient for computing running averages",
    )
    group.add_argument(
        "--izanagi-momentum",
        type=float,
        default=0.9,
        help="Coefficient for computing momentum",
    )
    group.add_argument(
        "--izanagi-eps",
        type=float,
    ...
    group.add_argument(
        ...
        type=int,
        default=5,
        help="Window size to use when sampling positives for the token alignment objective",
    )
    group.add_argument(
        "--taco-num-samples",
        type=int,
        default=50,
        help="Number of samples to use when sampling negatives for the token alignment objective",
    )
    group.add_argument(
        "--taco-temperature",
        type=float,
        default=0.07,
        help="Temperature to use when dividing logits for the token alignment objective",
    )
    group.add_argument(
        "--taco-loss-weight",
        type=float,
        default=1.0,
        help="Loss weighting for the token alignment objective",
    )
    group.add_argument(
        "--ct-loss-weight",
        type=float,
        default=1.0,
        help="Loss weighting for the contrastive token objective",
    )
    ...
    group.add_argument(
        ...
        type=int,
        default=1024,
        help="Size of the gate in the Mixture-of-Softmaxes output head",
    )
    group.add_argument(
        "--mixtape_dropout",
        type=float,
        default=0.3,
        help="Dropout percentage in the Mixture-of-Softmaxes output head",
    )
    ...
    group.add_argument(
        ...
        type=float,
        default=1.0,
        help="Temperature to use when sampling probabilities during adversarial ELECTRA training",
    )
