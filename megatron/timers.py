...
class Timers:
    ...
    def __call__(self, name, log_level=None):
        ...
        if name in self._timers:
            if log_level is not None:
                ...
                ), f"input log level {log_level} does not match \
                    already existing log level {self._log_levels[name]} \
                    for {name} timer"
            ...
            for rank in range(torch.distributed.get_world_size()):
                if rank_name_to_time[rank, i] > 0:
                    ...
                    if not_yet_found:
                        not_yet_found = False
                        output_string += f"\n  {name}:"
                    output_string += f"\n     rank {rank:2d}: {rank_name_to_time[rank, i] / normalizer:.2f}"
    ...
    def log(self, names, iteration, normalizer=1.0, reset=False, barrier=False):
        """Write timers to Weights & Biases
        Note that we only report maximum time across ranks.
        """
        assert normalizer > 0.0
        name_to_min_max_time = self._get_global_min_max_time(
            names, reset, barrier, normalizer
        ...
        for name in name_to_min_max_time:
            ...
            wandb.log({f"{name}-time": max_time, "iteration": iteration})
