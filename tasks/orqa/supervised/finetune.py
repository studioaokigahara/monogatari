...
def check_and_append_tensor_for_gather(group, rank, world_size, input_):
    ...
    first_dim = torch.tensor(
        [[current_length]], device=get_accelerator().current_device()
    )
...
def orqa(Dataset):
    def cross_entropy_forward_step(batch, model):
        ...
        return (
            output_tensor,
            partial(cross_entropy_loss_func, query_tokens, context_tokens),
        )
    ...
    def cross_entropy_loss_func(query_tokens, context_tokens, output_tensor):
        ...
        return (
            loss,
            {"lm loss": reduced_loss[0], "correct_prediction_count": reduced_loss[1]},
        )
