...
const SillyTavernPrompt = z.object({
    ...
    identifier: z.string(),
    injection_position: z.union([z.literal(0), z.literal(1)]).default(0), // 0 = “relative” (array order), 1 = “absolute”
    injection_depth: z.number().default(4),
    injection_order: z.number().max(10_000).default(100), // priority, default 100, max 10 000
...
export const SillyTavernPresetConverter = SillyTavernPreset.transform((preset) => {
    ...
    const listOrder: SillyTavernOrder["order"] =
        preset.prompt_order?.find?.((order) => String(order.character_id) === "100001")?.order ??
        [];
    ...
    const fieldsToSkip = new Set(["chatHistory", "enhanceDefinitions", "nsfw"]);
    ...
    listOrder.forEach((item) => {
        ...
        const createMessage = (content: string) => ({
            ...
            position:
                listOrder.findIndex((o) => o.identifier === item.identifier) >
                listOrder.findIndex((o) => o.identifier === "chatHistory")
                    ? ("after" as const)
                    : prompt.injection_position === 1
                      ? ("after" as const)
                      : ("before" as const),
            depth: prompt.injection_position === 0 ? 0 : (prompt.injection_depth ?? 0)
        });
