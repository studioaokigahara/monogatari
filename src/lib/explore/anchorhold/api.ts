const BASE_URL =
    "https://api.allorigins.win/raw?url=https://partyintheanchorhold.neocities.org";
...
function parseAnchorholdPosts(html: string, page: number): AnchorholdPost[] {
    ...
    const posts = Array.from(document.querySelectorAll(".post")).map(
        (postElement, index) => {
            ...
            const contentClone = contentElement?.cloneNode(true) as
                | HTMLDivElement
                | undefined;
            ...
            if (contentClone) {
                for (const a of Array.from(
                    contentClone.querySelectorAll("a")
                )) {
                    links.push({
...
export async function fetchAnchorholdConfig(): Promise<AnchorholdConfig> {
    ...
    if (!response.ok) {
        throw new Error(
            `Failed to fetch Anchorhold config.json: ${response.status} ${response.statusText}`
        );
    }
    ...
    if (
        typeof config.total_pages !== "number" ||
        typeof config.total_bots !== "number" ||
        typeof config.last_update !== "string"
    ) {
...
export async function fetchAnchorholdPage(
    page: number
): Promise<AnchorholdPost[]> {
    const response = await fetch(`${BASE_URL}/feed/page_${page}`);

    if (!response.ok) {
        throw new Error(
            `Failed to fetch page ${page} of Anchorhold feed: ${response.status} ${response.statusText}`
        );
    }
